...
To make your map, create 1 Powerpoint slide (e.g., HeatplotSkeleton.pptx) that contains these elements:
-Metabolites: Placed as Text Boxes where you would like them to appear containing the metabolite name. Note that these names need to match exactly those in the input data file and name conversion file.
...
Column 1: metabolite names, which must match those in the map coordinates file and name conversion file exactly
...
Column 1: metabolite full name, which must match those in the input data file and map coordinates file exactly
Column 2: the nickname you want displayed (if missing, the full metabolite name will be displayed)
