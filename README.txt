...
To make your map, create 1 Powerpoint slide (e.g., G3map.pptx) that contains these elements:
-Metabolites: Placed as Text Boxes where you would like them to appear containing the metabolite name. Note that these names need to match exactly (including capitalization case) those in the input data file and name conversion file. For Ji Seul's map, he used nicknames rather than exact names in the text boxes, so after running hotCoordinatesEdges.py on G3map.pptx I had to convert them to the exact names separately. Don't just edit Ji Seul's map and rerun hotCoordinatesEdges.py without consulting with Jamie.
...
3. map coordinates file (e.g., hardcodedCoordinatesRenamed.txt)
...
"coordfile": "C:\\Users\\jamie\\Dropbox (Personal)\\hotMetab\\hardcodedCoordinatesRenamed.txt"
}
...
python -m heatMetab -p "pathAndJsonFileName.json" -i "pathAndInfileName.pptx"
